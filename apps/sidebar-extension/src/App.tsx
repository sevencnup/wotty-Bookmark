...
function App() {
  ...
  return <main className="app-shell self-hosted-library">
    <header className="topbar"><div className="brand-lockup"><img alt="WOTTY BOOKMARK" className="brand-logo-img" src="/logo.png" /><div><div className="brand-title">书签库</div><div className="brand-subtitle">WOTTY · SERVER LIBRARY</div></div></div><button className={`sync-status ${connection ? 'is-connected' : 'is-disconnected'}`} onClick={() => setModal({ type: 'connect' })} type="button"><span className="status-dot" />{connection ? '服务器已连接' : '连接服务器'}<Server size={13} /></button></header>
    ...
    <footer className="app-footer"><span><span className="footer-dot" /> 服务器是唯一数据源</span><span className="footer-version">v0.2.0</span></footer>
