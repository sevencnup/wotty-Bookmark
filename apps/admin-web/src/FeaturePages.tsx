...
export function TrashPage({ token, navigate }: PageProps) {
  ...
  return <div className="feature-page">
    <PageIntro eyebrow="RECYCLE BIN" title="回收站" description="暂存从明文书签索引中移除的书签。恢复或删除操作会在下一次 Floccus 同步时写回浏览器。" action={<button className="toolbar-button" disabled={loading} onClick={() => void load()} type="button"><RefreshCw size={14} /> 刷新</button>} />
    {notice && <PageAlert tone="success"><strong>{notice}</strong></PageAlert>}
    {error && <PageAlert>{error}</PageAlert>}
    {loading ? <section className="panel feature-loading"><LoaderCircle className="spin" size={28} /><strong>正在读取回收站…</strong><p>正在确认当前同步文件的可用状态。</p></section> : data?.status === 'encrypted' ? <section className="panel capability-state"><ShieldCheck size={28} /><h3>当前同步文件已加密</h3><p>服务器无法解密或识别单个书签，因此不会展示旧索引内容，也不能在这里执行删除和恢复。请在浏览器或 Floccus 中管理书签。</p><button className="primary-button" onClick={() => navigate('floccus')} type="button">查看 Floccus 配置</button></section> : data?.status !== 'ready' ? <section className="panel capability-state"><ArchiveRestore size={28} /><h3>还没有可管理的书签索引</h3><p>完成一次明文 XBEL 同步后，回收站才可以提供条目级恢复。当前文件状态：{data?.status === 'notReady' ? '等待同步' : '格式待处理'}。</p><button className="primary-button" onClick={() => navigate('floccus')} type="button">前往同步配置</button></section> : data.items.length === 0 ? <section className="panel feature-empty"><ArchiveRestore size={28} /><h3>回收站是空的</h3><p>从书签整理页移除的书签会显示在这里。</p><button className="ghost-button" onClick={() => navigate('bookmark-organizer')} type="button">返回书签整理</button></section> : <section className="panel">
      <div className="panel-heading"><div><p className="eyebrow">{data.items.length} ITEMS</p><h3>已删除的书签</h3></div><button className="danger-button" onClick={() => setConfirm({ kind: 'empty' })} type="button"><Trash2 size={14} /> 清空回收站</button></div>
...
export function DevicesPage({ token, navigate, onSessionRevoked }: PageProps & { onSessionRevoked: () => void }) {
  ...
  return <div className="feature-page"><PageIntro eyebrow="DEVICES" title="设备管理" description="管理已登记的 Bookmark Vault 客户端设备。官方 Floccus 使用应用密码连接，会单独列在下方。" action={<button className="toolbar-button" disabled={loading} onClick={() => void load()} type="button"><RefreshCw size={14} /> 刷新</button>} />{error && <PageAlert>{error}</PageAlert>}<section className="panel"><div className="panel-heading"><div><p className="eyebrow">BOOKMARK VAULT CLIENTS</p><h3>已登记设备</h3></div></div>{loading ? <div className="feature-loading compact-loading"><LoaderCircle className="spin" size={24} /><span>正在读取设备…</span></div> : devices.length === 0 ? <div className="feature-empty compact-empty"><Laptop size={26} /><h3>还没有登记设备</h3><p>打开后台后，当前浏览器会自动登记为一个设备。</p></div> : <div className="device-list">{devices.map((device) => <article className={`device-card ${device.id === currentId ? 'current' : ''} ${device.revokedAt ? 'revoked' : ''}`} key={device.id}><span className="device-icon"><Laptop size={20} /></span><div className="device-content"><div className="device-title"><strong>{device.name}</strong>{device.id === currentId && <span className="current-badge">当前设备</span>}{device.revokedAt && <span className="revoked-badge">已撤销</span>}</div><p>{device.userAgentSummary || 'Bookmark Vault 管理后台'} · {device.deviceType}</p><small>最近活动 {formatDate(device.lastSeenAt)} · 登记于 {formatDate(device.createdAt)}</small></div>{!device.revokedAt && <button className="danger-button compact" disabled={busyId !== null || device.id === currentId} onClick={() => setConfirmId(device.id)} type="button">{device.id === currentId ? '当前设备' : '远程撤销'}</button>}</article>)}</div>}</section><section className="panel"><div className="panel-heading"><div><p className="eyebrow">WEBDAV CONNECTIONS</p><h3>应用密码连接</h3></div></div><div className="security-notice"><span>i</span><p>Floccus 不会调用设备登记接口，因此这里的应用密码是连接凭据，不等同于真实设备。你可以在密码管理页按浏览器分别撤销。</p></div><button className="primary-button" onClick={() => navigate('app-passwords')} type="button">管理应用密码</button></section>{confirmId && <ConfirmDialog danger confirmLabel="确认撤销" description="撤销后，该设备关联的后台会话会立即失效，需要重新连接。" loading={busyId !== null} onCancel={() => setConfirmId(null)} onConfirm={() => void revoke(confirmId)} title="撤销这个设备？" />}</div>
}
...
const helpArticles = [
  ...
  { id: 'pairing', title: '侧边栏连接码如何使用？', keywords: '侧边栏 连接码 pairing', content: '在 Floccus 配置页生成一次性连接码，复制到 Bookmark Vault 侧边栏的连接设置中。连接码 10 分钟内有效且只能使用一次，连接后不会保存应用密码或登录密码。' },
...
export function AboutPage() {
  ...
  useEffect(() => { void load() }, [])
  return <div className="feature-page"><PageIntro eyebrow="ABOUT BOOKMARK VAULT" title="关于项目" description="一个面向私人服务器部署的书签同步服务，使用官方 Floccus 和 WebDAV 完成跨浏览器同步。" action={<button className="toolbar-button" disabled={loading} onClick={() => void load()} type="button"><RefreshCw size={14} /> 刷新状态</button>} /><section className="about-hero panel"><span className="about-logo"><Fingerprint size={30} /></span><div><h3>Bookmark Vault</h3><p>让书签同步回到你自己的服务器。后台负责账户、应用密码、设备和存储状态；Floccus 负责浏览器书签同步。</p></div></section><div className="about-grid"><section className="panel"><div className="panel-heading"><div><p className="eyebrow">SERVICE STATUS</p><h3>服务状态</h3></div><span className={`service-status ${health ? 'online' : 'offline'}`}><i />{loading ? '检查中…' : health ? '运行正常' : '无法连接'}</span></div><dl className="about-details"><div><dt>API 服务</dt><dd>{health?.service ?? '—'}</dd></div><div><dt>API 版本</dt><dd>{health?.version ?? '—'}</dd></div><div><dt>访问地址</dt><dd>{window.location.origin}</dd></div></dl>{!health && !loading && <p className="form-error">无法连接服务，请确认 Rust API 正在运行。</p>}</section><section className="panel"><div className="panel-heading"><div><p className="eyebrow">ARCHITECTURE</p><h3>当前架构</h3></div></div><div className="architecture-list"><div><RefreshCw size={17} /><span><strong>Floccus + WebDAV</strong><small>使用官方客户端同步 XBEL 文件</small></span></div><div><ShieldCheck size={17} /><span><strong>客户端加密</strong><small>服务器不保存 Floccus passphrase</small></span></div><div><Laptop size={17} /><span><strong>浏览器侧边栏</strong><small>只操作当前浏览器的原生书签</small></span></div></div></section></div><section className="panel about-note"><ExternalLink size={17} /><p>Bookmark Vault 仍在持续开发中。标签、服务端解密和自有同步协议属于后续能力，不会在当前 Floccus 加密模式下伪装成已支持功能。</p></section><p className="workspace-note">管理后台版本 0.1.0 · 当前页面不会收集书签内容</p></div>
}
