...
export function AboutPage() {
  ...
  useEffect(() => { void load() }, [])
  return <div className="feature-page"><PageIntro eyebrow="ABOUT WOTTY BOOKMARK" title="关于项目" description="一个面向私人服务器部署的书签同步服务，使用官方 Floccus 和 WebDAV 完成跨浏览器同步。" action={<button className="toolbar-button" disabled={loading} onClick={() => void load()} type="button"><RefreshCw size={14} /> 刷新状态</button>} /><section className="about-hero panel"><span className="about-logo"><Fingerprint size={30} /></span><div><h3>WOTTY BOOKMARK</h3><p>让书签同步回到你自己的服务器。后台负责账户、应用密码、设备和存储状态；Floccus 负责浏览器书签同步。</p></div></section><div className="about-grid"><section className="panel"><div className="panel-heading"><div><p className="eyebrow">SERVICE STATUS</p><h3>服务状态</h3></div><span className={`service-status ${health ? 'online' : 'offline'}`}><i />{loading ? '检查中…' : health ? '运行正常' : '无法连接'}</span></div><dl className="about-details"><div><dt>API 服务</dt><dd>{health?.service ?? '—'}</dd></div><div><dt>API 版本</dt><dd>{health?.version ?? '—'}</dd></div><div><dt>访问地址</dt><dd>{window.location.origin}</dd></div></dl>{!health && !loading && <p className="form-error">无法连接服务，请确认 Rust API 正在运行。</p>}</section><section className="panel"><div className="panel-heading"><div><p className="eyebrow">ARCHITECTURE</p><h3>当前架构</h3></div></div><div className="architecture-list"><div><RefreshCw size={17} /><span><strong>Floccus + WebDAV</strong><small>使用官方客户端同步 XBEL 文件</small></span></div><div><ShieldCheck size={17} /><span><strong>客户端加密</strong><small>服务器不保存 Floccus passphrase</small></span></div><div><Laptop size={17} /><span><strong>浏览器侧边栏</strong><small>只操作当前浏览器的原生书签</small></span></div></div></section></div><section className="panel about-note"><ExternalLink size={17} /><p>WOTTY BOOKMARK 仍在持续开发中。标签、服务端解密和自有同步协议属于后续能力，不会在当前 Floccus 加密模式下伪装成已支持功能。</p></section><p className="workspace-note">管理后台版本 0.1.0 · 当前页面不会收集书签内容</p></div>
}
