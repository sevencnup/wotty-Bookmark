...
import { Bookmark, CheckCircle2, Folder, FolderInput, HardDriveUpload, LoaderCircle, Plus, RefreshCw, RotateCcw, Search, Server, Trash2 } from 'lucide-react'
...
export function LibraryManagementPage({ token }: { token: string }) {
  ...
  return <div className="feature-page library-page">
    ...
    <section className="panel library-list-panel">
      <div className="panel-heading"><div><p className="eyebrow">SERVER LIBRARY</p><h3>{library?.bookmarks.length ?? 0} 条书签 · {folders.length} 个文件夹</h3></div><button className="toolbar-button" disabled={loading || busy} onClick={() => void load()} type="button"><RefreshCw className={loading ? 'spin' : ''} size={15} />刷新</button></div>
      <label className="bookmark-search library-search"><Search size={15} /><input onChange={(event) => setQuery(event.target.value)} placeholder="搜索书签、网址或文件夹" type="search" value={query} /></label>
      {loading ? <div className="feature-loading compact-loading"><LoaderCircle className="spin" size={25} /><strong>正在读取服务器书签库…</strong></div> : visibleBookmarks.length ? <div className="library-table">{visibleBookmarks.map((bookmark) => <article className="library-bookmark-row" key={bookmark.id}><span className="library-bookmark-icon"><Bookmark size={16} /></span><div><strong>{bookmark.title}</strong><a href={bookmark.url} rel="noreferrer" target="_blank">{bookmark.url}</a></div><span className="category-pill">{bookmark.folderPath || '根目录'}</span><button aria-label={`删除 ${bookmark.title}`} className="row-action danger-action" disabled={busy} onClick={() => { if (window.confirm(`将“${bookmark.title}”移入服务器书签库回收站？`)) void run(() => api.deleteLibraryNode(token, bookmark.id), '已移入服务器书签库回收站') }} type="button"><Trash2 size={15} /></button></article>)}</div> : <div className="feature-empty compact-empty"><Bookmark size={25} /><h3>{query ? '没有匹配的书签' : '服务器书签库还是空的'}</h3><p>{query ? '请更换搜索关键词。' : '可在侧边栏收藏当前页，或从上方迁移 / 导入书签。'}</p></div>}
    </section>
