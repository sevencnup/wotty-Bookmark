...
export function LibraryManagementPage({ token }: { token: string }) {
  ...
  return <div className="feature-page library-page">
    ...
    <section className="panel library-list-panel">
      ...
      {loading ? <div className="feature-loading compact-loading"><LoaderCircle className="spin" size={25} /><strong>正在读取服务器书签库…</strong></div> : visibleBookmarks.length ? <VirtualBookmarkList bookmarks={visibleBookmarks} favicons={favicons} busy={busy} onDelete={(bookmark) => { if (window.confirm(`将“${bookmark.title}”移入服务器书签库回收站？`)) void run(() => api.deleteLibraryNode(token, bookmark.id), '已移入服务器书签库回收站') }} /> : <div className="feature-empty compact-empty"><Bookmark size={25} /><h3>{query ? '没有匹配的书签' : '服务器书签库还是空的'}</h3><p>{query ? '请更换搜索关键词。' : '可在侧边栏收藏当前页，或从上方迁移 / 导入书签。'}</p></div>}
    </section>
...
function VirtualBookmarkList({ bookmarks, favicons, busy, onDelete }: { bookmarks: api.LibraryBookmark[]; favicons: Record<string, string>; busy: boolean; onDelete: (bookmark: api.LibraryBookmark) => void }) {
