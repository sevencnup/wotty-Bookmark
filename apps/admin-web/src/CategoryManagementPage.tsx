          ...
          {visibleBookmarks.length === 0 ? <div className="category-empty"><Search size={25} /><strong>没有匹配的书签</strong><p>换个搜索关键词，或先在右侧选择其他文件夹。</p></div> : bookmarkGroups.map((group) => {
            ...
            return <section className={`category-bookmark-group ${collapsed ? 'collapsed' : ''}`} key={group.id}>
              <div className="category-bookmark-group-header organization-folder-row" style={{ display: 'grid', gridTemplateColumns: '24px 26px minmax(0, 1fr) auto', alignItems: 'center', paddingLeft: `${12 + Math.min(group.depth, 4) * 10}px` }}>
                ...
                <button aria-expanded={!collapsed} aria-label={`${collapsed ? '展开' : '收起'} ${group.path}`} className="category-group-toggle organization-toggle" onClick={() => toggleGroup(group.id)} type="button">{collapsed ? <ChevronRight size={15} /> : <ChevronDown size={15} />}</button>
                <button className="category-group-summary organization-folder-button" onClick={() => toggleGroup(group.id)} type="button"><span className="category-group-icon">{collapsed ? <Folder size={15} /> : <FolderOpen size={15} />}</span><span><strong>{group.title}</strong><small title={group.path}>{group.path}</small></span></button>
